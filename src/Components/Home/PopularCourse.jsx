        ...
        <div className="w-full 2xl:h-[100vh] bg-white shadow-lg rounded-md p-4 flex flex-col gap-3">
            <div className="w-full lg:flex items-center justify-between">
                <div className="text-center lg:text-start mb-3 lg:mb-0">
                    ...
                    <h2 className="text-3xl font-bold text-[var(--color-primary)]">Popular Courses</h2>
                </div>
                <div className="flex gap-3 xl:gap-6 bg-white sm:justify-between">
                    <NavLink onClick={()=> setSelect('All')} className={`px-2 md:px-4 py-1 md:py-2 ${select == "All" && 'bg-primary text-[var(--color-secondary)]'} text-[var(--color-primary)] flex items-center`}>All</NavLink>
                    <NavLink onClick={()=> setSelect('Data Science')} className={`px-2 md:px-4 py-1 md:py-2 ${select == 'Data Science' && 'bg-primary text-[var(--color-secondary)]'} text-[var(--color-primary)] flex items-center`}>Data Science</NavLink>
                    <NavLink onClick={()=> setSelect('Cyber Security')} className={`px-2 md:px-4 py-1 md:py-2 ${select == 'Cyber Security' && 'bg-primary text-[var(--color-secondary)]'} text-[var(--color-primary)] flex items-center`}>Cyber Security</NavLink>
                    <NavLink onClick={()=> setSelect('Development')} className={`px-2 md:px-4 py-1 md:py-2 ${select == 'Development' && 'bg-primary text-[var(--color-secondary)]'} text-[var(--color-primary)] flex items-center`}>Development</NavLink>
                    <NavLink onClick={()=> setSelect('Graphic Design')} className={`px-2 md:px-4 py-1 md:py-2 ${select == 'Graphic Design' && 'bg-primary text-[var(--color-secondary)]'} text-[var(--color-primary)] flex items-center`}>Graphic Design</NavLink>
                </div>
            </div>
            ...
            <div className="w-full h-[86%] flex gap-4 bg-[var(--color-secondary)] p-3 rounded-md">
                ...
                >
                    ...
                    })).map((course, idx) => <SwiperSlide key={idx} className="w-full h-full bg-white rounded-md pb-4">
                    {/* Card Image Section */}
                    <div className="w-full h-[45%] mb-3">
                        <img src={courseBanner1} alt="" className="w-full h-full rounded-md" />
                    </div>
                    {/* Card Content Section */}
                    <div className="w-full h-[52%] px-3 flex flex-col gap-1">
                        <h1 className="text-lg xl:text-2xl font-bold">{course.name}</h1>
